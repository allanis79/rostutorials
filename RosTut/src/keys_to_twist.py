...
class Key_to_twist:
	...
	def callback(self,msg):
		if len(msg.data) == 0 or not self.dict.has_key(msg.data[0]):
			return

		vels = self.dict[msg.data[0]]
		self.g_last_twist = Twist()
		self.g_last_twist.angular.z = vels[0]
		self.g_last_twist.linear.x = vels[1]
		self.pub.publish(self.g_last_twist)
...
def keys_cb(msg):
	global g_last_twist, g_vel_scales, g_target_twist
